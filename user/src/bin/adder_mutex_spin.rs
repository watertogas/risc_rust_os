...
use user_lib::{thread_exit, get_time, thread_create, waittid};
...
unsafe fn f() -> ! {
    let mut t = 2usize;
    let mutex_id = MUTEX_ID;
    for _ in 0..PER_THREAD {
        mutex_lock(mutex_id);
        critical_section(&mut t);
        mutex_unlock(mutex_id);
    }
    thread_exit(t as i32)
}
...
pub fn main(argc: usize, argv: &[&str]) -> i32 {
    ...
    //spinlock id begins with 64;
    unsafe {
        MUTEX_ID = mutex_create() as usize;
        assert_eq!(MUTEX_ID, 64);
    }
