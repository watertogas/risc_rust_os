...
use crate::task::process::set_syscall_return_value;
...
lazy_static! {
    static ref LISTEN_TABLE: OneCoreCell<Vec<Option<Port>>> =
        unsafe { OneCoreCell::new(Vec::new()) };
}
...
// can accept request
pub fn accept(listen_index: usize, task: TaskID) {
    let mut listen_table = LISTEN_TABLE.exclusive_access();
...
pub fn accept_connection(_port: u16, tcp_packet: &TCPPacket, task: TaskID) {
    ...
    let fd = set_new_fd(task.to_pid(), Arc::new(tcp_socket));
    set_syscall_return_value(task, fd)
